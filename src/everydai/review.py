...
def main(imgdir, reviewdir, datestart, datefinish, sleepstart=0, sleepfinish=0, puttemplate=False, extension='.jpg'):
    ...
    if needsreview == 0:
        print('All dates have already been reviewed from', datestart.strftime('%Y-%m-%d'), 'to',
              (datefinish - timedelta(days=1)).strftime('%Y-%m-%d') + '.')
    elif needsreview == 1:
        print(str(needsreview), 'day needs reviewing from', datestart.strftime('%Y-%m-%d'), 'to',
              (datefinish - timedelta(days=1)).strftime('%Y-%m-%d') + '.')
    else:
        print(str(needsreview), 'days need reviewing from', datestart.strftime('%Y-%m-%d'), 'to',
              (datefinish - timedelta(days=1)).strftime('%Y-%m-%d') + '.')
    if needlog:
        log.close()
